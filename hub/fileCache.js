...
module.exports = (function() {
        ...
        var transmitFile = function(i, cb) {
          ...
          fs.readFile(file, { encoding: "utf8", flag: "r"}, function(err, fileData) {
            if (err) {
              ...
              console.log(util.isArray(JSON.parse(fileData).d));
              var tmpfileData = "";
              if (fileData.length > 0) {
                tmpfileData = fileData;
                if(util.isArray(JSON.parse(fileData).d)){
                  tmpfileData = "";
                  for(var j=0;j<JSON.parse(fileData).d.length;j++){
                    var tmpData = {
                      "id":JSON.parse(fileData).id,
                      "d":JSON.parse(fileData).d[j]
                    };
                    if(tmpfileData.length>0){
                      tmpfileData +=",";
                    }
                    tmpfileData += JSON.stringify(tmpData);
                  }
                }
  ...
  FileCache.prototype.cacheThis = function(dataIn) {
